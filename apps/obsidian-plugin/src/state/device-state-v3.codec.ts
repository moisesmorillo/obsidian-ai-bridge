import {
  ...
  type ReconciliationPreservationReceiptV3,
...
export const MAX_MIRROR_DEVICE_STATE_V3_BYTES = 8 * 1024 * 1024;

/** Strict host-local decode result; incompatible data is never treated as missing. */
export type MirrorDeviceStateV3DecodeResult =
  | { readonly kind: "missing" }
  | { readonly kind: "valid"; readonly state: MirrorDeviceStateV3 }
...
const deviceStateSchema = z
  .object({
    ...
    version: z.literal(MIRROR_DEVICE_STATE_V3_VERSION),
    deviceId: z.string(),
    lifecycle: lifecycleSchema,
    globalBlockReason: z
      .enum([
...
export async function decodeMirrorDeviceStateV3(
  ...
  if (
    header.success &&
    header.data.version !== MIRROR_DEVICE_STATE_V3_VERSION
  ) {
  ...
  let state: MirrorDeviceStateV3;
  try {
    state = convertDeviceState(decoded.data);
  } catch {
    return { kind: "corrupt" };
  }
  if (!isMirrorDeviceStateV3Consistent(state)) return { kind: "corrupt" };
 ...
 */
export function encodeMirrorDeviceStateV3(state: MirrorDeviceStateV3): string {
  if (!isMirrorDeviceStateV3Consistent(state)) {
  ...
  /* v8 ignore next -- projection from a validated v3 domain state is schema-total. */
  ...
  try {
    ...
    if (!isMirrorDeviceStateV3Consistent(convertDeviceState(validated.data))) {
  ...
  if (byteLength(encoded) > MAX_MIRROR_DEVICE_STATE_V3_BYTES) {
 ...
 */
function projectDeviceState(state: MirrorDeviceStateV3): DeviceStateDto {
  return {
    format: MIRROR_DEVICE_STATE_FORMAT,
    version: MIRROR_DEVICE_STATE_V3_VERSION,
 ...
 */
function convertDeviceState(dto: DeviceStateDto): MirrorDeviceStateV3 {
  return {
...
function convertMatchingPrecondition(
  ...
  /* v8 ignore start -- matching-revision callers exclude the absent schema branch. */
...
function convertReconciliationPathEvidence(
  ...
  switch (dto.remote.kind) {
    ...
    case RECONCILIATION_REMOTE_EVIDENCE_KIND.live: {
      ...
      /* v8 ignore start -- the live-evidence schema accepts content receipts only. */
      if (receipt.action === MUTATION_ACTION.tombstone) {
    ...
    case RECONCILIATION_REMOTE_EVIDENCE_KIND.tombstone: {
      const receipt = convertOperationReceipt(dto.remote.receipt);
      /* v8 ignore start -- the tombstone-evidence schema requires a tombstone receipt. */
      if (receipt.action !== MUTATION_ACTION.tombstone) {
...
function convertTransferableAcknowledgement(
  ...
  /* v8 ignore start -- staged-handoff entries exclude unassociated acknowledgements. */
