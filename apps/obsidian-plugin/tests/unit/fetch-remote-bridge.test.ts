...
function admission(): RemoteRequestAdmission & {
  ...
  const release = vi.fn();
  return { admit: vi.fn().mockResolvedValue({ release }), release };
}
...
function acknowledgement(action: "create" | "update" = "create") {
  return {
    path: PATH,
    revision: REVISION,
    receipt:
...
describe("FetchRemoteBridge", () => {
  ...
  it("keeps the deadline through a body stall and settles without waiting for a non-cooperative body", async () => {
    let cancelled = false;
  ...
  it("times out a Fetch that never resolves after invocation and maps its mutation effect as unknown", async () => {
    const { bridge, requestAdmission } = adapter(
  ...
  it("adapts recovery metadata, content withholding, and explicit maintenance responses", async () => {
    ...
    await expect(bridge.sealRecovery(maintenance)).resolves.toMatchObject({
      kind: "confirmed",
      confirmed: { kind: "sealed", id: RECOVERY },
    });
    await expect(bridge.purgeRecovery(maintenance)).resolves.toMatchObject({
      kind: "confirmed",
      confirmed: { kind: "sealed", id: RECOVERY },
    });
