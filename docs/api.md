...
This implementation is local/development evidence, not proof of a deployment, bucket, production operation, or connected Obsidian client. The plugin runtime still makes no Worker calls; Slice 4's typed Fetch adapter is intentionally uncomposed, and Slice 5 is the next internal M3 work.
