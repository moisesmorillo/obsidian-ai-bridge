...
serialized operation admission. Slice 3 adds uncomposed, operation-authorized local
create/replace and durable conflict-preservation primitives with postcondition proof,
unknown-effect recovery, and persistence fencing. It adds no UI, action orchestration,
remote mutation, tombstone/restore/history execution, timer, Fetch, or Worker/API
behavior.
...
| Local safety | Shared literal `.md` path and 1 MiB policy; dot-prefixed/configuration-directory exclusions; pre-read metadata and post-read UTF-8 bound; exact lookup and pre/post object/path/mtime/size checks. M4 Slice 3's separate uncomposed adapter supports only official lookup/create/createFolder/read/process operations, fixed generated conflict paths, create-only collision rules, atomic exact-text replacement, and post-effect identity/hash verification. No delete/rename/move, raw filesystem, path repair/URI decoding, content UI, or cross-system atomicity claim. | `packages/core/src/local-vault/`, `apps/obsidian-plugin/src/infrastructure/`, focused adapter tests |
...
- Vitest **5**: **69 source test files / 1002 tests**, plus **1 generated-artifact
...
- Root Vitest projects include shared packages, Worker and plugin. M4 Slice 3 source
  coverage is statements **95.05%**, branches **91.39%**, functions **98.22%**, lines
  **97.03%**; thresholds and production inclusion remain enforced. Artifact tests are separate
...
Slices 1–3's contract/state/migration, core read-only review/admission, and uncomposed
local write/preservation seams implemented. No M4 UI, action orchestration, or remote
mutation exists; resolution, restore, history, composition, and qualification slices
remain unimplemented. The accepted M4 design is reviewed-only: exact format-2 revisions
