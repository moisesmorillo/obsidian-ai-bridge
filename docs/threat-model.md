...
This document consolidates the implemented and qualified M1–M5 trust and safety
boundary. M5's final report closes A1–A12 for the latest v1.0.2 release within one
exact Obsidian/macOS desktop profile and a 10,000-note synthetic active-writer
ceiling. The report is software qualification, not security certification,
penetration-test evidence, production deployment, or complete-backup assurance. M6
remains an unimplemented MCP adapter with open design decisions.
...
Each row distinguishes implemented controls from the completed M5 qualification evidence
and residual risk. The M5 report links the corresponding A1–A12 evidence.
...
| --- | --- | --- | --- | --- |
| Unauthorized plaintext reads | Notes, recovery content; client → Worker → R2 | Strict registry auth, one exact operation/permission policy, private R2, canonical paths, bounded bodies, sanitized errors, and `no-store` responses. | [A4/A7](qualification/m5-final.md#a4--permission-completeness) live permission and content-free-log checks; Worker integration matrix. | Worker/Cloudflare/host operators and correctly authorized readers remain trusted with plaintext; no application E2E encryption. Owner: service operator and trusted-platform providers. |
| Unauthorized mutation or deletion | Current/recovery state; client → Worker | Independent `write`/`delete`, exact writer/association guards, conditional revisions/receipts, recovery-first tombstones, and typed application policy. | [A4/A8](qualification/m5-final.md#a8--recovery-and-rollback) exact read-only refusal and retained recovery after rejected purge; exhaustive route tests. | Authorized writers can exercise granted authority; application rules cannot infer operator intent. Owner: credential-granting operator. |
...
| Excessive reads or scans | R2 availability/cost and plaintext exposure; client → Worker/R2 | Bounded payloads/pages/cursors, retired v1 aggregation, `read` permission, and independent client revocation. | [A6](qualification/m5-final.md#a6--bounded-operation) active-writer 1k/5k/10k and current v5 restart matrix; no quota/limiter is configured. | Authorized volumetric abuse and platform/account exhaustion remain possible; no SaaS-scale or DoS guarantee. Owner: bridge operator and Cloudflare account owner. |
...
| Secret or sensitive log leakage | Credentials, note/recovery bodies, paths and metadata; runtime/log boundaries | Structured live diagnostics exclude names, secrets/digests, headers, bodies, concrete/encoded identifiers, revisions, hashes, receipts, storage envelopes and raw exceptions; zero-day application retention. | [A7](qualification/m5-final.md#a7--diagnostics-and-privacy) actual loopback event inspection plus leakage integration tests. | Live sampling/missed sessions leave no history; privileged runtime/platform compromise can observe transient plaintext. Owner: service operator and platform providers. |
...
| Untrusted Markdown interpreted as instructions or active content | Note/recovery text; remote → plugin/human/future agent | Literal-text review controls, no Markdown/HTML rendering or command execution, bounded validation, and explicit untrusted-content guidance. | [A8/A10](qualification/m5-final.md#a10--architecture) artifact hostile-text tests and loopback content-free diagnostic assertions. | Other trusted plugins or humans may interpret imported content; future MCP needs its own prompt boundary. Owner: host operator and future MCP implementer. |
...
compromise. Other platforms/event traces remain unqualified. ADR 0011's no-quota,
zero-day-log, manual-recovery, latest-only, and v1-retirement choices remain binding;
see the [final M5 report](qualification/m5-final.md) for measured evidence and limits.
