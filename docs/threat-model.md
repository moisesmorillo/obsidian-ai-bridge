...
accepted boundary. M5 Slice 2 now implements the credential registry, typed principal,
offline lifecycle, and singleton migration checkpoint; route permission enforcement
remains Slice 4. It is a design and prioritization artifact, not a
...
| --- | --- | --- | --- | --- | --- |
| Unauthorized plaintext reads | Notes, recovery content; internet/client → Worker → R2 | API descendants require one strict registry bearer and resolve a named typed principal; credentials are independently revocable; private R2 has no client credentials; canonical paths, bounded bodies, sanitized errors, and `no-store` responses apply. | Enforce the principal's `read` permission through the exhaustive route table and remove the temporary singleton migration mode. | Worker/Cloudflare/host operators and a correctly authorized read client remain trusted with plaintext; no application E2E encryption is planned. | Slice 4 route authorization |
| Unauthorized mutation or deletion | Current/recovery state; client → Worker | Strict registry authentication, independent client revocation, designated association/writer UUID checks, exact conditional revisions/receipts, recovery-first tombstones, retired v1 mutations, preservation, and typed application policy fail closed. | Principals carry exact permissions, but those permissions are not yet enforced per route; every authenticated client therefore retains current route authority in Slice 2. | A client intentionally granted `write` or `delete` can exercise that authority after Slice 4; application invariants limit effects but cannot infer operator intent. | Slice 4 permission table and enforcement |
| Credential replay after disclosure | Client credentials; any network location → Worker | HTTPS is required outside exact loopback development; raw tokens stay in approved client secret stores; the Worker keeps a bounded digest-only registry, resolves a typed principal, and supports independent offline rotation/revocation/loss replacement. | Remove the temporary singleton migration mode in Slice 4 and add client-attributed live diagnostics in Slice 3. | Bearer credentials remain replayable until verified revocation; M5 adds no proof-of-possession or hardware binding. | Slices 3–4 diagnostics and migration completion |
| Excessive reads or scans | R2 availability/cost, plaintext exposure, logs; client → Worker/R2 | Payloads and v2 pages are bounded; cursors are opaque/bounded; v1 aggregation currently has a finite 50,000-object scan ceiling; named clients can be independently revoked. | Enforce read permission, retire v1, and qualify existing hard bounds at the 10,000-note target. ADR 0011 selects no application quota or limiter binding. | Authorized volumetric abuse and platform/account exhaustion remain accepted personal-bridge risks; no SaaS-scale or denial-of-service guarantee is claimed. | Slice 4 authorization/retirement, then Slice 6 qualification |
| Excessive destructive requests | Current/recovery integrity and cost; authorized client → Worker/R2 | CAS, operation IDs, association continuity, recovery-before-tombstone, permanent tombstones/markers, idempotent exact replay, and independent client revocation constrain each operation. | Enforce independent `delete` authorization, then exercise hard/effect-certainty bounds. ADR 0011 selects no numeric destructive quota. | A fully authorized delete client can deliberately issue excessive valid operations until revocation/platform controls take effect; application invariants protect each effect, not availability or cost. | Slice 4 authorization, then Slice 6 qualification |
| Secret or sensitive log leakage | Credentials, note/recovery bodies, paths, metadata; all runtime/log boundaries | Structured logs omit raw authorization, bodies, concrete note paths, raw exceptions, and storage envelopes; plugin state/status/handoff omit tokens and bodies; artifact tests include leakage negatives. | Add client-attributed live diagnostic fields and qualify redaction. ADR 0011 selects zero-day application-log retention and no audit-log claim. | Missed/sampled live diagnostics may leave no history; platform/operator metadata visibility remains inside the trust boundary; arbitrary host/runtime compromise can observe transient plaintext. | Slice 3 live diagnostics, then Slice 6 qualification |
...
| Unsafe code/config/credential rollback | Device state, revoked credentials, format-2 state; operator/deployment boundary | Strict v2→v3→v4 migration, incompatible runtime refusal, no reverse migration, v1 mutation retirement, and forward-only operator guidance. | Prevent/qualify restoration of revoked digests, synchronize supported artifacts, remove v1 reads, and exercise latest-only forward-upgrade/refusal runbooks. | Platform rollback controls are operator responsibilities; no claim that every stale but valid snapshot is detectable or that an unqualified rollback is supported. | Slice 4 retirement, Slice 5 runbooks, Slice 6 qualification |
