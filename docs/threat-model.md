...
| Unsafe code/config/credential rollback | Device state, revoked credentials, format-2 state; operator/deployment boundary | Strict v2→v3→v4→v5 migration, frozen historical codecs, incompatible runtime refusal, no reverse migration, complete v1 HTTP retirement, and forward-only operator guidance. | Prevent/qualify restoration of revoked digests, synchronize supported artifacts, and exercise latest-only forward-upgrade/refusal runbooks against v5. | Platform rollback controls are operator responsibilities; no claim that every stale but valid snapshot is detectable or that an unqualified rollback is supported. | Slice 5 runbooks, Slice 6 qualification |
| Missed local events across listener gaps | Note state, M3/M4 reservations and effects; Obsidian Vault → plugin runtime | Listeners attach after layout readiness; startup callbacks are bounded/buffered; cold starts and fresh epochs durably gap-fence active v5 operations; dispatch-time listener/configuration leases guard effects; complete-group reviewed settlement/transfer retains reservations on unknown or incomplete evidence. Positive scan absence never authorizes deletion. | Qualify the full desktop failure matrix and 10,000-note envelope; exercise iCloud ordering, overflow, and repeated attach/detach on supported releases. | Host event delivery is not a transaction log; unreviewable evidence may retain reservations and require operator attention. | M4 ADR 0013 correction; M5 Slice 6 qualification |
