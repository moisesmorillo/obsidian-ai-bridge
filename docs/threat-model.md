...
| Excessive reads or scans | R2 availability/cost, plaintext exposure, logs; client → Worker/R2 | Payloads and v2 pages are bounded; cursors are opaque/bounded; v1 aggregation currently has a finite 50,000-object scan ceiling. | Implement scoped revocation and retire v1; qualify existing hard bounds at the 10,000-note target. ADR 0011 selects no application quota or limiter binding. | Authorized volumetric abuse and platform/account exhaustion remain accepted personal-bridge risks; no SaaS-scale or denial-of-service guarantee is claimed. | Slices 2–4 credentials/retirement, then Slice 6 qualification |
| Excessive destructive requests | Current/recovery integrity and cost; authorized client → Worker/R2 | CAS, operation IDs, association continuity, recovery-before-tombstone, permanent tombstones/markers, and idempotent exact replay constrain each operation. | Implement independent `delete` authorization and revocation, then exercise hard/effect-certainty bounds. ADR 0011 selects no numeric destructive quota. | A fully authorized delete client can deliberately issue excessive valid operations until revocation/platform controls take effect; application invariants protect each effect, not availability or cost. | Slices 2–4 authorization/revocation, then Slice 6 qualification |
